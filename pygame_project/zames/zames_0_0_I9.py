...
def do_trees(trees_dict, change, center):
    ...
    center[0] += change[pygame.K_a] - change[pygame.K_d]
    center[1] += change[pygame.K_w] - change[pygame.K_s]    
    ...
    if not trees_dict:
        ...
        for i in range(0, 200, step):
            delta = random.randrange(i, i + step) * math.pi / d
            x_tree, y_tree = tree_x_y(X, Y, radius, delta)
            trees_dict[(x_tree, y_tree, i, step, d)] = {"point": [x_tree, y_tree], "hit_box": [10, 40], "hit_flag": False, "seed": True}
    ...
    for t in trees_dict.copy():
        ...
        if min(map(lambda point_1, point_2: abs(point_1 - point_2), trees_dict[t]["point"], (X, Y))) < min((X, Y)) and trees_dict[t]["seed"]:
            ...
            for i in range(1, 3):
                delta = random.randrange(i, i + step) * math.pi / d
                r
                x_tree, y_tree = tree_x_y(center[0], center[1], r, delta)
                trees_dict[(x_tree, y_tree, delta, r)] = {"point": [x_tree, y_tree], "hit_box": [10, 40], "hit_flag": False, "seed": True}
