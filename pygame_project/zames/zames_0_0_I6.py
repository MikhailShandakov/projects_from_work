...
def do_trees(trees_dict, change):
    ...
    radius = random.randrange(step // 2, step * 3 // 4)
    
    if not trees_dict:
        
        for i in range(0, 20, 5):
            delta = random.randrange(i, i + 5) * 0.1 * math.pi
            x_tree, y_tree = tree_x_y(X, Y, radius, delta)
            trees_dict[(x_tree, y_tree, i)] = {"point": [X + x_tree, Y + y_tree], "hit_box": [10, 40], "hit_flag": False}
            
    
    for t in trees_dict:
        trees_dict[t]["point"][0] += change[pygame.K_LEFT] - change[pygame.K_RIGHT]
        trees_dict[t]["point"][1] += change[pygame.K_UP] - change[pygame.K_DOWN]
...
def moving(change_dict):
    for event in pygame.event.get():
        if event.type == pygame.KEYDOWN:
            change_dict[event.key] = 1
        elif event.type == pygame.KEYUP:
            change_dict[event.key] = 0
...
def gameloop():
    ...
    move_dict = {pygame.K_UP: 0, pygame.K_DOWN: 0, pygame.K_LEFT: 0, pygame.K_RIGHT: 0}
