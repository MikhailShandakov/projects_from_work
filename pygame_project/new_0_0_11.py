...
def gameloop():
    ...
    bullet_dict = {}
    tree_dict = {(0, 1): [random.randrange(WIDTH), random.randrange(HEIGHT // 2 - 50)],
                 (0, -1): [random.randrange(WIDTH), random.randrange(HEIGHT // 2 + 50, HEIGHT)],
                 (1, 0): [random.randrange(WIDTH // 2 + 50, WIDTH), random.randrange(HEIGHT)],
                 (-1, 0): [random.randrange(WIDTH // 2 - 50), random.randrange(HEIGHT)]}
...
#                     up, down, left, right = False, False, False, False
        ...
        if count_up > SHIFT_UP:
            count_up = 0
            tree_dict[(0, count)] = [random.randrange(WIDTH), random.randrange(-SHIFT_UP, 0)]
            
        if count_down > SHIFT_DOWN:
            count_down = 0
            tree_dict[(0, count)] = [random.randrange(WIDTH), random.randrange(HEIGHT, HEIGHT + SHIFT_UP)]
            
        if count_left > SHIFT_LEFT:
            count_left = 0
            tree_dict[(count, 0)] = [random.randrange(-SHIFT_LEFT, 0), random.randrange(HEIGHT)]
            
        if count_right > SHIFT_RIGHT:
            count_right = 0
            tree_dict[(count, 0)] = [random.randrange(WIDTH, WIDTH + SHIFT_RIGHT), random.randrange(HEIGHT)]
